...
struct FileReader : Reader {
  ...
  virtual int read_next_char() { return getc_unlocked(fp); }
};
...
struct MemWriter : Writer {
  ...
  MemWriter(char *data_, size_t size_)
      : data(data_), size(size_), p(0), got_error(false) {}
  virtual bool valid() { return true; }
  virtual size_t pos() { return p; }
  ...
  void _overflow_error(size_t more) {
    fprintf(
        stderr,
        "MemWriter, overflowing to the buffer (pos %li, size %li, add %li)\n",
        p, size, more);
    got_error = true;
  }
};
...
public:
  bool got_error;
  Parser(Reader *reader_, Writer *writer_)
      : reader(reader_), writer(writer_), got_error(false) {}
...
public:
  ...
  void parse_dict_or_set() {
    ...
    while (true) {
      ...
      if (c == ',') {
        ...
        if (cur == Key) {
          parse_error("dict after parsing key", c);
          return;
        }
        if (obj_type == Dict)
          cur = Key;
        ...
        if (cur != Key) {
          parse_error("dict expected key before", c);
          return;
        }
        if (obj_type == Set) {
          parse_error("set", c);
        ...
        parse_error("dict|set", c);
        return;
      }
    }
    if (obj_type == Dict && count % 2 != 0) {
      parse_error("dict, uneven count", c);
      return;
    }
  ...
  void parse_none(char first) {
    if (!('o' == read_next_char() && 'n' == read_next_char() &&
          'e' == read_next_char())) {
...
#ifdef LIB
extern "C" int py_to_pickle(const char *in, size_t in_len, char *out,
                            size_t out_len) {
