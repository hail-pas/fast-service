...
class CBV:
    ...
    def __init__(self, common_param: int, z: int = Depends(dependency)):
        self.y = 1
        self.z = z

    @api_router.get("/cbv/", response_model=Resp)
    def f(self) -> int:
        return Resp(data=self.cx + self.x + self.y + self.z)

    @api_router.get("/cbv/classvar", response_model=Resp)
    def g(self) -> bool:
        return Resp(data=hasattr(self, "cy"))
